...
|---|---|---|---|---|---|
| **2 — Global tokens** ✅ merged-pending (PR #19) | `fix/phase2-global-tokens` | `static/css/style.css`, `templates/base.html` | G1 contrast, G2 focus ring, G3 skip-link/landmarks, G4 active-nav, **G5 heading discipline (CSS foundation ONLY — see deferral note below)**, G6 penalty reframe, G8 est-purse chip token, G9 44px floor, G10 flash role + field-error scaffold, G11 money treatment, U1 mobile points total | `/audit`, `/polish`, `/harden`, `/typeset`, `/colorize`, `/adapt` | Re-critique a representative core-loop page (e.g. `index`) to confirm tokens propagate AND no regression on an already-good surface. **Must merge before Phases 3–5.** Detector delta 13→3 (all remaining = G5 tag fixes deferred to P3/P5). |
| **3 — Core loop** | `fix/phase3-core-loop` | `index.html`, `make_pick.html`, `tournament_detail.html`, `my_picks.html` (+ page CSS) | G7 mobile money pill (per-surface); **G5 heading-tag fixes DEFERRED FROM PHASE 2** — make_pick `<h4>…<h6>` skip (missing h5), my_picks `<h2>…<h5>` skip (missing h3/h4); also audit index `<h2>…<h5>` "Season Standings → League Rules" skip. Swap to a sequential level; the Phase-2 card-header CSS is already level-agnostic so this is a visual-neutral tag change. Each clears one `skipped-heading` detector finding; U2 legend/timestamp/backup-text/next-pick CTA; U4 mobile gold pill, collapse no-pick rows, multiplier-alert contrast; U5 row-status binding, live-row link, legend money-code, mobile CTA height; U3 used-player cue, inline submit confirm, mobile rules | `/adapt`, `/harden`, `/clarify`, `/layout`, `/distill`, `/onboard`, `/audit` | Re-`critique` each of the 4 pages; confirm Nielsen `/40` moved up vs the unit's recorded score; record deltas. Harness target for make_pick/my_picks: `skipped-heading` → 0. |
